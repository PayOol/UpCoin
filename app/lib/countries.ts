...
export const ALL_COUNTRIES: CountryInfo[] = [
  ...
  { code: "TC", name: "Îles Turks-et-Caïcos", dialCode: "+1649", flag: "🇹🇨" },
  { code: "VG", name: "Îles Vierges britanniques", dialCode: "+1284", flag: "🇻🇬" },
  { code: "VI", name: "Îles Vierges des États-Unis", dialCode: "+1340", flag: "🇻🇮" },
...
export const dialCodes: Record<string, string> = Object.fromEntries(
...
export async function detectUserCountry(): Promise<{ countryCode: string; dialCode: string } | null> {
  const sources: (() => Promise<IpLookupResult>)[] = [
    ...
    async () => {
      ...
      const data = (await res.json()) as { country_code?: string; country_calling_code?: string; error?: boolean };
  ...
  for (const fetchSource of sources) {
    try {
      ...
      if (result && result.countryCode) {
        const resolvedDial = result.dialCode || dialCodes[result.countryCode];
        if (resolvedDial) {
          return { countryCode: result.countryCode, dialCode: resolvedDial };
        }
      }
    } catch {
      // try next source
